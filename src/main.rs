...
/____/_/ /_/\__,_/_.___/_.___/_/|_|\__,_/_/   \__/\__,_/  
    ...
    let middle_of_map = Wgs84::from_sweref(
        &Sweref { north: (max_y + min_y)*0.5, east: (max_x + min_x)*0.5, });
    ...
    let northeast_corner = Wgs84::from_sweref(&Sweref { north: max_y, east: max_x, });
    let southwest_corner = Wgs84::from_sweref(&Sweref { north: min_y, east: min_x, });

    if verbose {
        println!("[{}] Height over sea level: {:.0} m", &module, height_over_sea_level);
